...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    void (^block)(MFMailComposeViewController *controller, MFMailComposeResult result, NSError *error) = objc_getAssociatedObject(self, "blockCallbackTwo");
...
- (id)initWithCompletionHandler:(void (^)(MFMailComposeViewController *controller, MFMailComposeResult result, NSError *error))block {
    self = [super init];
    if (self) {
        objc_setAssociatedObject(self, "blockCallbackTwo", [block copy], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
