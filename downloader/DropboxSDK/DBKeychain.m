...
+ (NSDictionary *)credentials {
	...
	if (status == noErr && foundValue) {
		...
		if (status != errSecItemNotFound) {
			DBLogWarning(@"DropboxSDK: error reading stored credentials (%i)", (int)status);
		}
		return nil;
	}
}
...
+ (void)setCredentials:(NSDictionary *)credentials {
	...
	if (status != noErr) {
		DBLogWarning(@"DropboxSDK: error saving credentials (%i)", (int)status);
	}
}
...
+ (void)deleteCredentials {
	...
	if (status != noErr) {
		DBLogWarning(@"DropboxSDK: error deleting credentials (%i)", (int)status);
	}
}
